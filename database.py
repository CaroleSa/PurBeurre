...
class Database:
    ...
    def load_insert_data(self):
        ...
        for elt, element in zip(self.categories, self.list_data):
            ...
            insert_data_categories = ("""INSERT IGNORE INTO Category (categories) VALUES({0});""".format("\'"+elt+"\'"))
            ...
            for value in element['products']:
                if element['products'].index(value) < 100:
                    try:
                        ...
                        insert_data_food = ("""INSERT IGNORE INTO Food (name_food, category_id, nutriscore, 
                                            description, store, link) 
                                            VALUES({0}, (SELECT id FROM Category WHERE categories = {1}),
                                            {2}, {3}, {4}, {5});"""
                                            .format(product_name, "\'"+elt+"\'", nutrition_grade, ingredients,
                                            store_tags, url))
